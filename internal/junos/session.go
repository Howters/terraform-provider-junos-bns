...
func genSSHClientConfig(auth *sshAuthMethod) (*ssh.ClientConfig, error) {
	...
	configs[0].HostKeyCallback = hostKeyCallback
...
func hostKeyCallback(hostname string, remote net.Addr, key ssh.PublicKey) error {
    ...
    log.Printf("Received key from %s: %s %s", 
        hostname, 
        key.Type(), 
        base64.StdEncoding.EncodeToString(fingerprint[:]))
    
    // Here you would typically verify the key against a known good value
    // For now, we'll accept the key but strongly recommend verifying it
    log.Printf("WARNING: Accepting unverified host key for %s. Please verify this key manually.", hostname)
